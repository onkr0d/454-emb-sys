...
int main() {
  ...
  // configuration of joystick registers according to Lab Manual 4.6.2, "Button
  // #1 is located on PORTE PIN8..."
  ...
  while (1) {
    oldtime = TMR3; // Start timing the loop
    // FIXED very funny edge cases:
    // 1. when minutes > 0, and 0 < seconds < 10, the ms timer displays 4
    // spaces, ie, xxxx, instead of xxx
    // 2. after you reset it with the trigger (and seconds > 10), the above
    // case changes to minutes >= 0. how is this possible? there should be no
    // possible values > 1000, seeing as we mod % 1000. only god knows.
    ...
    // Display every 25000 iterations but using the per-iteration measurements
    if (iters++ == 25000) {
      iters = 0;
      lcd_locate(0, 0);
      lcd_printf("Last reset: %02lu:%02lu.%03lu",
                 (milliseconds / (1000UL * 60UL)), (milliseconds / 1000UL) % 60,
                 milliseconds % 1000UL);
      lcd_locate(0, 1);
      lcd_printf("cycles: %d  %.4f", clk_cycles, ms_time);
    }
  }
